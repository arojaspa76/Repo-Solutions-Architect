...
def chat(q: Query):
    ...
    ids = hits.get("ids", [[]])[0] if hits else []
    distances = hits.get("distances", [[]])[0] if hits else []
    ...
    # Distancia coseno - similitud
    scores = [(1 - d) for d in distances] if distances else []

    # Si se quieeere ver el porcentaje
    scores_pct = [round(s * 100, 1) for s in scores]
    ...
    # aqui se debe devolver las fuentes con los scores
    sources = []
    for _id, meta, doc, s, sp in zip(ids, metas, docs, scores, scores_pct):
        sources.append({
            "id": _id,
            "score": round(s, 4),       # similitud coseno [0..1]
            "score_pct": sp,            # 0..100
            "file": meta.get("file"),
